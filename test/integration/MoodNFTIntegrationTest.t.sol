// SPDX-License-Identifier: MIT
pragma solidity ^0.8.25;

import {Test} from "forge-std/Test.sol";
import {console2} from "forge-std/console2.sol";
import {MoodNFT} from "../../src/MoodNFT.sol";
import {DeployMoodNFT} from "../../script/DeployMoodNFT.s.sol";

contract MoodNFTIntegrationTest is Test {
    MoodNFT private moodNFT;
    DeployMoodNFT private deployMoodNFT;

    string private sadSVGImageURI =
        "data:image/svg+xml;base64,";
    string private happySVGImageURI =
        "data:image/svg+xml;base64,";

    address USER = makeAddr("user");

    function setUp() public {
        deployMoodNFT = new DeployMoodNFT();
        moodNFT = deployMoodNFT.run();
    }

    /**
     * * @dev This test checks the integration of the mintNFT function and the tokenURI function.
     * It mints an NFT and then retrieves the token URI to ensure that the minting process works correctly.
     */
    function testViewTokenURIIntegration() public {
        vm.startPrank(USER);
        moodNFT.mintNFT();
        string memory tokenURI = moodNFT.tokenURI(0);
        console2.log(tokenURI);
        // assertEq(moodNFT.s_tokenCounter(), 1);
        vm.stopPrank();
    }

    /**
     * @dev This test checks the integration of the flipMood function and the tokenURI function.
     * It mints an NFT, flips its mood, and then retrieves the token URI to ensure that the mood flipping process works correctly.
     */
    function testFlipMoodIntegration() public {
        vm.startPrank(USER);
        moodNFT.mintNFT();

        vm.startPrank(USER);
        moodNFT.flipMood(0);
        string memory tokenURI = moodNFT.tokenURI(0);
        console2.log(tokenURI);
        vm.stopPrank();

        assertEq(
            keccak256(abi.encodePacked(tokenURI)),
            keccak256(abi.encodePacked(sadSVGImageURI))
        );

        // assertEq(tokenURI, sadSVGImageURI);
    }
}
